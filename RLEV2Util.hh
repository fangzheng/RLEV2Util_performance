/*
 * This file is copied from Apache ORC's codebase and slightly modified for test purpose.
 */
...
namespace orc {
  ...
  // enum definition copied from RLEv2.hh
  ...
  inline uint32_t decodeBitWidth_new(uint32_t n) {
    return FBSToBitWidthMap[n];
  }

  inline uint32_t getClosestFixedBits_new(uint32_t n) {
    if (n <= 64) {
  ...
  inline uint32_t getClosestAlignedFixedBits_new(uint32_t n) {
    if (n <= 64) {
  ...
  inline uint32_t encodeBitWidth_new(uint32_t n) {
    if (n <= 64) {
